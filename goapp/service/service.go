...
import (
	...
	"github.com/mlctrez/go-nats-app/api"
...
func (s *Service) Start(_ service.Service) (err error) {
	...
	if s.ns, err = startNats(); err != nil {
		return
	}

	if err = setupApiEndpoints(engine, s.ns); err != nil {
		return
	}
...
func setupApiEndpoints(engine *gin.Engine, server *server.Server) error {

	api.New(server).Register(engine)
...
func startNats() (ns *server.Server, err error) {
	opts := &server.Options{
		ServerName:     "Your friendly backend",
		Host:           "127.0.0.1",
		Port:           -1, // pick a random port and use ns.ClientURL() for connecting
		NoLog:          true,
		NoSigs:         true,
		MaxControlLine: 4096,
	...
	ns.SetLogger(logger, false, false)

	go ns.Start()

	if !ns.ReadyForConnections(4 * time.Second) {
		ns.Shutdown()
		err = fmt.Errorf("nats not ready")
	}

	ns.SetLogger(nil, false, false)
